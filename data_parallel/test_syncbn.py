...
def init_process(rank, size, fn, inputs, hid_dim, backend='gloo'):
    ...
    fn(rank, size, inputs, hid_dim)


def sync_batch_norm(rank, size, inputs, hid_dim, momentum=0.1, eps=1e-5):
...
@pytest.mark.parametrize(["batch_size"], [128, 256, 512, 1024])
...
def test_batchnorm(num_workers, hid_dim, batch_size):
    ...
    sync_inputs.requires_grad = True
    worker_inputs = [sync_inputs[ind:ind + batch_size] for ind in range(0, batch_size, batch_size // num_workers)]
    worker_dims = [hid_dim] * num_workers
    worker_size = [num_workers] * num_workers
    worker_fn = [sync_batch_norm] * num_workers
